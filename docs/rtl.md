...
boring, but we're look at some interesting parts.

The current target is Tang Primer 20K FPGA board. But the code is fairly
portable, you can adapt it to any target with DVI output by changing the wrapper
around `raycast_top` module.

I was exploring synthesizable SystemVerilog during the creation of this project,
so currently the code is verified to work with
[yosys-slang](https://github.com/povik/yosys-slang) frontend plugin for Yosys.
It uses some features of the language, like `type()` operator, which native
Yosys frontend can't handle.

We'll use 32x32 textures, 32x32 square map with 7 textures. The screen
resolution is classic 480x640. Because we only need to calculate 640 rays,
there's no reason to make the whole raycaster pipelined, it would just stay
stalled most of the time, and it's a huge resource waste in terms of gates or
LUTs. To make the design a bit more challenging, I tried to focus on making it
...
In the code above, a synthesizer like Yosys is able to recognize that `sel`
...
file and running `proc` and `clean` passes. We need `proc` to replace
`always_comb` process cell with a multiplexer cell and `clean` to get rid of 1
...
And now let's run `share` pass to merge multipliers and then `opt` to simplify
the logic a bit:

![design after share](../img/design_after_share.png)

After `share` pass we have the following log:
...
It found that `$mul` cells could be merged based on shared control signal.

## Fixed point

One of the most challenging parts of implementing this raycaster was to make an
algorithm design for floating point numbers to work with fixed point.
...
This fixed point representation is very convinient, because you don't have to
pass additional metadata with a data object to determine integer and fractional
parts, the data object stores this information in itself. You can get integer
amount of bits with `$left(foo) + 1` and fractional amount with `-$right(foo)`.

This is very neat for fixed point operations, which I'll describe later.

### Types

I started with fixed point Python model and just tried different bit widths to
...
unsigned` type, fractional width has to be casted to be `signed` before making
it negative, otherwise we'll have a very large positive value in 2's complement
form, because in SV, the result is unsigned of any of the operands is unsigned.

Ray direction vector has a length of 1, with a value in range [-1, 1], so 2 bits
is enough for the integer part. Fractional part could be reduced even further,
but the texture quality would drop significantly.

The largest value position data type can hold is the ray distance, which for
32x32 map is diagonal $\sqrt{32^2 + 32^2} \approx 45.3$, which won't fit in 5
integer bits, but my map doesn't have large open spaces, so the rays are short
enough to fit. For an open map, interger bit width has to be increased.
...
Side type is for position inside single map square, so the value won't exceed 1.

Extended position is needed for calculating delta step between cell edges, if
the ray is almost parallel, that value is large and will overflow 5 bits, 8 bits
provide extra precision and prevent most overflows. If the result is larger than
8 bits, than it gets clipped to the maximum value.
...
total size, so the multiplication result is 18 bits. But what if we don't want
to increase the width every time we multiply? For simple integers, we just
truncate from MSB. But in case of fixed point, multiplication resulted in 8
...
The algorithm is to get expanded multiplication result, shift bits to the right
by the amount of fractional bits in the original operand type, and then cast it
to the operand type to truncate bits from the left. Tedious to do that by hand
all the time, so I created a macro:
...
The neat part is that it only requires to pass 2 operands (although of the same
type) and no additional information. `type()` operator allows to get a type of
the operand and then cast the result to it.

The multiplication above just truncates fractional bits, which results in worse
...
And my favourite macro is that casts fixed point number of one type to the other
type:
...
First, we are shifting the number to change fractional amount of bits. If we
...
all bits of 1 and see if the result is less than 0: `type(num)'('1) < 0`. Which
is a very cool trick, so
...
You might wonder why I use concatenation `{}` curly braces with only one
element. It is done to isolate bit length of macro expressions from outer
expression context. Concatenation operands are self-determined (IEEE 1800-2023
11.6.1), so expressions inside and outside the macro have fully separate
contexts and won't influence each other. Otherwise, macro expressions might
result in unexpected expansion of other operands. The side effect is that the
result of concatenation is always unsigned (IEEE 1800-2023 11.8.1), so the
resulting value has to be explicitly casted to be signed if needed.

## Hierarchy

The main parts of the raycaster are render, controls and DVI modules. The whole
...
The biggest problem with the raycaster is that even though ray value is
calculated only once for the screen column, the screen driver goes from left to
right. So every pixel clock cycle we need to provide data from the different
ray.
...
values, so with single frame buffer there will be screen tearing, as the buffer
updates during frame drawing. So we have to use twice as much memory to
...
and writes it to SRAM and pipeline that reads from SRAM and performs final
...
elaboration time. All of those places are in ray calculation FSM, so the
...
reciprocal of the denominator and then multiply it by numerator.
...
has the best initial convergence, when the input value is in [0.5, 1] range.
...
no 1 bits in the integer part. After iterating over the value, we left shift it
again to counter the initial shift. With the initial estimate of 1, 8 iterations
are enough to get the proper result, more iterations doesn't seem to improve the
image quality, as the fixed point precision itself becomes the bottleneck.

You can find the full SV implemention in `rtl/newton_inv.sv`.
