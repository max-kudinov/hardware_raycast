...
localparam int unsigned W_CNT = 8;

typedef logic signed [W_CNT-1:0] cnt_t;
...
always_comb begin
    ...
    if (DE) begin
        if ((cnt == 0) || (N1_qm == N0_qm)) begin
            ...
            if (~q_m[8]) begin
                cnt_next = cnt + cnt_t'(N0_qm) - cnt_t'(N1_qm);
            end else begin
                cnt_next = cnt + cnt_t'(N1_qm) - cnt_t'(N0_qm);
            end
            ...
            if (((cnt > 0) && (N1_qm > N0_qm)) ||
                ...
                q_next[8]   = q_m[8];
                q_next[7:0] = ~q_m[7:0];
                cnt_next    = cnt + (cnt_t'(q_m[8]) << 1) +
                                     cnt_t'(N0_qm) - cnt_t'(N1_qm);
